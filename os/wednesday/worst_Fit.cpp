...
void allocateMemory(int processID, int size) {
    ...
    int allocStart = holes[worstIndex].start;
    allocations[processID] = {allocStart, size};

    if (holes[worstIndex].size == size)
        holes.erase(holes.begin() + worstIndex);
    else {
        holes[worstIndex].start += size;
        holes[worstIndex].size -= size;
    }
...
int main() {
    ...
    while (true) {
        cout << "\nMenu:\n";
        cout << "1. Allocate Memory (Worst Fit)\n";
